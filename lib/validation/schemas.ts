...
export const createPostSchema = z.object({
  ...
  scope: z.enum(['global', 'nearby', 'city']).default('global'),
  room_id: z.string().uuid().nullable().optional(),
  gif_url: z.string().url().nullable().optional(),
  feeling: z.string().max(50).nullable().optional(),
  feeling_emoji: z.string().max(10).nullable().optional(),
  activity: z.string().max(50).nullable().optional(),
  activity_emoji: z.string().max(10).nullable().optional(),
  activity_detail: z.string().max(100).nullable().optional(),
  location_name: z.string().max(200).nullable().optional(),
  is_life_event: z.boolean().optional(),
  life_event_type: z.string().max(50).nullable().optional(),
  life_event_emoji: z.string().max(10).nullable().optional(),
  neighborhood_id: z.string().uuid().nullable().optional(),
  reshared_from_id: z.string().uuid().nullable().optional(),
  reshare_comment: z.string().max(500).nullable().optional(),
  tagged_user_ids: z.array(z.string().uuid()).max(20).default([]),
}).refine(
  d => !!(d.content?.trim() || d.image_url || d.video_url || d.gif_url || d.is_life_event),
  { message: 'Post must have text, image, video, GIF, or life event' }
)
