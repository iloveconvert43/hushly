...
export default function FeedList({ filter, roomSlug, selectedCity }: Props) {
  ...
  // Refresh feed when a new post was just created (cross-page event)
  useEffect(() => {
    // Check sessionStorage on mount (fired before navigation completed)
    try {
      if (sessionStorage.getItem('hushly-feed-refresh')) {
        sessionStorage.removeItem('hushly-feed-refresh')
        firstPostId.current = null
        setNewPostCount(0)
        mutate()
      }
    } catch {}

    // Listen for the event (fired after navigation completes)
    function onFeedRefresh() {
      try { sessionStorage.removeItem('hushly-feed-refresh') } catch {}
      firstPostId.current = null
      setNewPostCount(0)
      mutate()
    }
    window.addEventListener('hushly-feed-refresh', onFeedRefresh)
    return () => window.removeEventListener('hushly-feed-refresh', onFeedRefresh)
