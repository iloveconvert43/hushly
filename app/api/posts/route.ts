...
export async function POST(req: NextRequest) {
  ...
  // Validate all fields via Zod schema
  const parsed = validate(createPostSchema, body)
  if (!parsed.success) {
  ...
  // Sanitize tags
  const validTags = Array.isArray(tags) ? sanitizeTags(tags) : []
  ...
  const { data: post, error } = await supabase.from('posts').insert({
    user_id:     profile.id,
    content:     trimmed ? sanitizeText(trimmed) : null,
    ...
    ...(activity     ? { activity, activity_emoji: activity_emoji || null,
                         activity_detail: activity_detail ? sanitizeText(activity_detail) : null } : {}),
    ...(location_name ? { location_name: sanitizeText(location_name) }     : {}),
  ...
  const pointsReason = hasMedia
    ? (is_mystery ? 'mystery_post' : 'media_post')
    : 'post_created'
